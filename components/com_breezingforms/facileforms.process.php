...
class HTML_facileFormsProcessor {
    ...
    var $sendNotificationAfterPaymentFromPiece = false;
...
// header
	...
	function view() {
        ...
        if($this->sendNotificationAfterPaymentFromPiece){

            JFactory::getSession()->set('sendNotificationAfterPaymentFromPiece', true);

        }else{

            JFactory::getSession()->set('sendNotificationAfterPaymentFromPiece', false);
        }
					...
					switch ($cbEntry->recType) {
						case 'File Upload':
							if (trim($this->formrow->template_code_processed) == 'QuickMode') {
								...
								if($cbOut){
                                    $cbOut = '<br/><div class="bfCbFileMessage">'.BFText::_("COM_BREEZINGFORMS_CHECK_TO_DELETE_FILES").'</div><br/>' . $cbOut;
                                }
								$js .= $cbDeac;
								$js .= '
                                                    if (document.createTextNode){
                                                        if(!document.getElementById("bfFlashFileQueue' . $cbEntry->recElementId . '")){
                                                           ...
                                                           mydiv.innerHTML = '.json_encode($cbOut).';
...
// sendMail
	...
	function exppdf($filter = array(), $mailback = false, $translate = true) {
        ...
        if(isset($matches_array[1]) && isset($matches_array[1][0]) && trim($matches_array[1][0]) == 'fm'
            ...
            if(file_exists($pdfname) && !$mailback){ // we assume mailback pdfs may have the same filename as their admin mail counterpart and do NOT use a different filename
...
// expxml
	...
	function sendEmailNotification() {
		...
		if (!$this->sendNotificationAfterPayment && !$this->sendNotificationAfterPaymentFromPiece && !JFactory::getSession()->get('sendNotificationAfterPaymentFromPiece', false)) {
			for ($i = 0; $i < $recipientsSize; $i++) {
				$this->sendMail($from, $fromname, $recipients[$i], $subject, $body, $attachment, $isHtml, null, null, $alt_sender);
			}
		} else if($this->sendNotificationAfterPayment || $this->sendNotificationAfterPaymentFromPiece || JFactory::getSession()->get('sendNotificationAfterPaymentFromPiece', false)){
					...
					switch ($row->type) {
						...
						case 'Signature':
							if ($row->logging == 1) {
								...
								$values = @JRequest::getVar("ff_nm_" . $row->name, array(''), 'POST', 'RAW', JREQUEST_ALLOWRAW);
