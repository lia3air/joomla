					...
					case 'bfRadioGroup':
						...
						if($mdata['group'] != ''){
							...
							for($i = 0; $i < $lines; $i++){
                                ...
                                $sep = ';';
                                $custom_sep = substr( $gEx[$i], 0, 1 ) === "~";
                                if($custom_sep){
                                    $custom_sep_ex = explode("~", $gEx[$i]);

                                    if(count($custom_sep_ex) == 3){
                                        $gEx[$i] = $custom_sep_ex[2];
                                        $sep = $custom_sep_ex[1];
                                    }
                                }

								$iEx = explode($sep, $gEx[$i]);
								$iCnt = count($iEx);
								if($iCnt == 3){
					...
					case 'bfSelect':
						...
						if($mdata['list'] != ''){
							...
							for($i = 0; $i < $lines; $i++){
								...
								$iCnt = count($iEx);
								if($iCnt == 3){
									echo '<option '.($iEx[0] == 1 ? 'selected="selected" ' : '').'value="'.htmlentities(trim($iEx[2]), ENT_QUOTES, 'UTF-8').'">'.trim($iEx[1]).'</option>'."\n";
								}
							}
							echo '</select>'."\n";
						}
